...
def game_search(message):
    ...
    try:
                ...
                for key, value_link in libs.items():
                    if game_data.lower() in key.lower():
                        ...
                        if i == True:
                            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
                            bot.send_message(message.chat.id,
                                             '*Если это не та игра ,которую вы ищите ,то нажмите "/link"*',
                                             parse_mode='Markdown')
        ...
        bot.send_message(message.chat.id, 'Вы неправильно ввели название игры или я её не нашёл,может скините мне ссылку на игру? Нажмите "/link"')
